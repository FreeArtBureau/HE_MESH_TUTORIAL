...
void setup() {
  ...
  for (int j = 0; j < 21; j++) {
    for (int i = 0; i < 21; i++) {
       frame.addNode(vertices[index][0],vertices[index][1],vertices[index][2],(i+1)*0.075);
       index++;
    }
  }
  ...
  for (int j = 0; j < 20; j++) {
    for (int i = 0; i < 20; i++) {
      if (random(100)>50) frame.addStrut(i+21*j, i+1+21*j);
      if (random(100)>50) frame.addStrut(i+21*j, i+21*(j+1));
    }
  }
  ...
  creator.setMaximumStrutOffset(12);// limit the joint radius by decreasing the strut radius where necessary. Joint offset is added after this limitation.
